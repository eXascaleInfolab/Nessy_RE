...
from vad_model.multi_dae import MultiDAE


class MultiTextVAE(MultiDAE):

    def __init__(self, p_dims, q_dims, config, embedding_matrix=None, lam=0.01, random_seed=31):
        ...
        super(MultiTextVAE, self).__init__(p_dims, q_dims, lam, lr, random_seed)

    def construct_placeholders(self):
        self.input_ph = tf.compat.v1.placeholder(
            dtype=tf.int32, shape=[None, self.dims[0]])
    ...
    def q_graph(self):
        ...
        # Embedding layer
        with tf.device(self.device), tf.compat.v1.variable_scope(
                "embedding", tf.compat.v1.AUTO_REUSE):
            E = tf.compat.v1.get_variable("E", shape=self.embedding_matrix.shape,
                                initializer=tf.constant_initializer(self.embedding_matrix))
            embedding_chars = tf.nn.embedding_lookup(E, self.input_ph)
            embedding_chars_flatten = tf.layers.flatten(embedding_chars)
    ...
    def _construct_weights(self):
        ...
        self.q_dims[0] = self.q_dims[0] * self.embedding_matrix.shape[1]
